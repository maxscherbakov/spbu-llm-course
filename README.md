...
### 1. Гибридный Парсинг (Hybrid Parsing)
*   Используется **PyMuPDF** для быстрого извлечения текста.
*   Интегрирован **RapidOCR (ONNX Runtime GPU)**. Система автоматически определяет страницы с таблицами-картинками (на основе площади изображений) и применяет OCR только к ним, сохраняя баланс между скоростью и качеством.
*   Надежное извлечение SHA1-хэша из имени файла.

### 2. Умная Индексация (Advanced Indexing)
...
### 3. Многоступенчатый Поиск (Advanced Retrieval)
